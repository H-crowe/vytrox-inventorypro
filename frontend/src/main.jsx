...
function InvoiceModal({ invoice, onClose }) {
  return <div className="modal-backdrop"><article className="invoice">
    <div className="invoice-head">
      <div className="invoice-brand"><div className="brand-mark"><Store size={20} /></div><div><strong>Vytrox InventoryPro</strong><span>Inventory and sales operations</span></div></div>
      <div className="invoice-actions"><button className="secondary icon-button" onClick={() => window.print()}><Printer size={17} />Print</button><button onClick={onClose}>Close</button></div>
    </div>
    <div className="invoice-meta">
      <div><p className="eyebrow">Invoice</p><h2>{invoiceNumber(invoice.sale.id)}</h2><span>Issued {formatDate(invoice.sale.created_at)}</span></div>
      <div><span>Prepared by</span><strong>{invoice.sale.username}</strong></div>
    </div>
    <div className="invoice-parties">
      <div><span>Bill To</span><strong>{invoice.sale.customer_name}</strong><p>{invoice.sale.phone || 'No phone provided'}</p></div>
      <div><span>Payment Summary</span><strong>{money(invoice.sale.total)}</strong><p>{invoice.items.length} line item{invoice.items.length === 1 ? '' : 's'}</p></div>
    </div>
    <div className="invoice-table">
      <table>
        <thead><tr><th>SKU</th><th>Item</th><th>Qty</th><th>Unit Price</th><th>Line Total</th></tr></thead>
        <tbody>{invoice.items.map((item) => <tr key={item.id}><td>{item.sku}</td><td>{item.name}</td><td>{item.quantity}</td><td>{money(item.unit_price)}</td><td>{money(Number(item.unit_price) * Number(item.quantity))}</td></tr>)}</tbody>
      </table>
    </div>
    <div className="totals"><span>Subtotal <strong>{money(invoice.sale.subtotal)}</strong></span><span>Discount <strong>{money(invoice.sale.discount)}</strong></span><strong>Total {money(invoice.sale.total)}</strong></div>
    <p className="invoice-note">Thank you for your business. This invoice was generated by Vytrox InventoryPro.</p>
  </article></div>;
}
...
function invoiceNumber(id) { return `INV-${String(id).padStart(5, '0')}`; }
